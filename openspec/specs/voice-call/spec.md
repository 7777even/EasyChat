...
单聊（好友）或群会话（同群成员）中一方 SHALL 能够发起音视频通话，在线成员 SHALL 收到来电提醒并可接听 / 拒绝 / 忙线。
...
- **THEN** 服务端校验 A 为 G 成员，向 G 在线成员广播 `call_invite`（`callType=2`, `groupId=G`），并建立通话房间；参与者数超过 `max-participants` 时服务端拒绝并提示
...
通话建立后，参与者 SHALL 经由 WebRTC P2P 直连传输音视频，并支持静音、关摄像头与挂断；状态多端实时同步（含群成员）。
...
所有 SDP/ICE SHALL 经由后端 Netty WS 中继，前端无需直连对方网络；TURN 保障对称 NAT 连通；失败有明确兜底。

#### Scenario: 信令中继与房间路由

- **WHEN** 一端生成 offer/answer 或收集到 ICE candidate
- **THEN** 以 `call_signal` 帧经 WS 发往服务端，服务端按通话房间路由给目标；`call_signal` 不落库、不触发普通消息逻辑
...
任一方挂断 / 取消 / 忙线 / 离会，相关方 SHALL 收到对应状态帧并正确结束浮窗、释放 `RTCPeerConnection` 与设备。
...
每次通话以任一方式结束后，服务端 SHALL 持久化一条 `call_log`，可供后续回溯。

#### Scenario: 落库时机与内容

- **WHEN** 一次通话以接听后挂断 / 未接 / 拒接 / 取消 / 忙线 任一种方式结束
- **THEN** 服务端依据通话房间写入一条 `call_log`：`caller_id`、`call_type`(1单聊/2群)、`peer_id`(单聊对方) 或 `group_id`(群)、`media_type`(1音频/2音视频)、`start_time`、`end_time`、`status`(1已接/2未接/3拒接/4取消/5忙线)、`participant_count`、`create_time`
