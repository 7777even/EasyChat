# Spec Delta — 群文件：上传 / 列表 / 删除 / 下载

- 关联 Tasks: 2026-09-26-group-file/tasks.md
- 创建日期: 2026-09-26

> 格式对齐 `openspec/specs/group-file/spec.md`。
...
- **WHEN** 非本群成员调用上传接口
- **THEN** 抛出 `CODE_2302`（不在群组中），不写入任何文件
...
- **THEN** 均被 `checkGroupRole(MEMBER)` 拦截，返回 `CODE_2302`
...
|---------------------|-------------------|-----------|
| C1 | ADDED: 群文件上传 | 2.1, 2.2 |
| C2 | ADDED: 群文件列表 | 2.2, 3.3, 3.4 |
| C3 | ADDED: 群文件删除 | 2.1, 2.2 |
| C4 | ADDED: 群文件最小权限 | 2.1, 2.2 |
